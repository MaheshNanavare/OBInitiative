...
- `src/data/team.ts`, `projects.ts`, `partners.ts`: typed arrays that import images. Team entries have optional `focus` (object-position) and `zoom` (scale), which `our-story.astro` uses to frame faces in circular crops.
